...
use BarCode\Error\BarcodeException;
use Cake\Controller\Component;
use BarCode\Error\InvalidCharacterException;
use BarCode\Error\InvalidCheckDigitException;
use BarCode\Error\InvalidFormatException;
use BarCode\Error\InvalidLengthException;
use BarCode\Error\UnknownTypeException;
...
class BarCodeComponent extends Component
{
     ...
     * @public
     */
    public function getBarcodePNG($code, $type, $widthFactor = 2, $totalHeight = 60, $color = array(0, 0, 0))
    {
     ...
     * @public
     */
    public function getBarcodeJPG($code, $type, $widthFactor = 2, $totalHeight = 60, $color = array(0, 0, 0))
    {
        $barcodeData = $this->getBarcodeData($code, $type);
        ...
        ob_start();
        header("Content-type: image/jpg"); // Automatic content type output
        if ($imagick && isset($imageMagickObject)) {
     ...
     * @public
     */
    public function getBarcodeSVG($code, $type, $widthFactor = 2, $totalHeight = 30, $color = 'black')
    {
        $barcodeData = $this->getBarcodeData($code, $type);

        // replace table for special characters
        $repstr = array("\0" => '', '&' => '&amp;', '<' => '&lt;', '>' => '&gt;');
        header("image/svg+xml"); // Automatic content type output
     ...
     * @public
     */
    public function getBarcodeHTML($code, $type, $widthFactor = 2, $totalHeight = 30, $color = 'black')
    {
        $barcodeData = $this->getBarcodeData($code, $type);
